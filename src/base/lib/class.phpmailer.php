...
class PHPMailer {
    ...
    protected function AddAnAddress($kind, $address, $name = '') {
        if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
            ...
            if ($this->exceptions) {
                throw new phpmailerException('invalid_recipient');
            }
            if ($this->SMTPDebug) {
        ...
        if (!self::ValidateAddress($address)) {
            ...
            if ($this->exceptions) {
                throw new phpmailerException('invalid_address');
            }
            if ($this->SMTPDebug) {
    ...
    protected function SmtpSend($header, $body) {
        ...
        if(!$this->smtp->Data($header . $body)) {
            throw new phpmailerException("SMTP Error: Data not accepted. " . json_encode($this->smtp->getError()), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
            ...
            throw new phpmailerException($badaddresses,'12306');
        }
        return true;
    }
